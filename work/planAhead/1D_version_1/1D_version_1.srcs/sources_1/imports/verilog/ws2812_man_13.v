/*
   This file was generated automatically by the Mojo IDE version B1.3.6.
   Do not edit this file directly. Instead edit the original Lucid source.
   This is a temporary file and any changes made to it will be destroyed.
*/

module ws2812_man_13 (
    input clk,
    input rst,
    input [39:0] data,
    output reg out
  );
  
  
  
  localparam NUM_LED = 4'ha;
  
  localparam LED_OFF = 2'h0;
  
  localparam LED_RED = 2'h1;
  
  localparam LED_GREEN = 2'h2;
  
  localparam LED_BLUE = 2'h3;
  
  localparam COLOR_BLACK = 24'h000000;
  
  localparam COLOR_RED = 24'h008000;
  
  localparam COLOR_GREEN = 24'h800000;
  
  localparam COLOR_BLUE = 24'h000080;
  
  reg [39:0] M_board_d, M_board_q = 1'h0;
  
  reg [13:0] M_counter_d, M_counter_q = 1'h0;
  
  reg [10:0] foo;
  
  reg [1:0] led_state;
  
  reg [23:0] grb;
  
  integer c;
  
  always @* begin
    M_counter_d = M_counter_q;
    M_board_d = M_board_q;
    
    out = 8'h00;
    for (c = 1'h0; c < 4'ha; c = c + 1) begin
      M_board_d[(c)*2+1-:2] = data[(c)*2+1-:2];
    end
    if (M_counter_q < 16'h3c00) begin
      c = M_counter_q / 12'h600;
      led_state = M_board_q[(c)*2+1-:2];
      
      case (led_state)
        2'h0: begin
          grb = 24'h000000;
        end
        2'h1: begin
          grb = 24'h008000;
        end
        2'h2: begin
          grb = 24'h800000;
        end
        2'h3: begin
          grb = 24'h000080;
        end
        default: begin
          grb = 24'h000000;
        end
      endcase
      foo = M_counter_q - ((M_counter_q / 12'h600) * 12'h600);
      if (grb[(5'h17 - (foo >> 3'h6))*1+0-:1] == 1'h0) begin
        if (foo[0+5-:6] < 5'h14) begin
          out = 1'h1;
        end
      end else begin
        if (foo[0+5-:6] < 6'h28) begin
          out = 1'h1;
        end
      end
      M_counter_d = M_counter_q + 1'h1;
    end
  end
  
  always @(posedge clk) begin
    if (rst == 1'b1) begin
      M_counter_q <= 1'h0;
    end else begin
      M_counter_q <= M_counter_d;
    end
  end
  
  
  always @(posedge clk) begin
    if (rst == 1'b1) begin
      M_board_q <= 1'h0;
    end else begin
      M_board_q <= M_board_d;
    end
  end
  
endmodule
